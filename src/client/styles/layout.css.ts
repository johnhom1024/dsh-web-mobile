...
@media (max-width: 1023px) and (pointer: coarse) {
  ...
  /* PermissionSelect / plan controls share the tools lane. Let the
     ...
     Anchor the slot by its semantic class, NOT by :nth-child(2): host
     0.1.5-rc added a second Tooltip-wrapped _add button (file.attach,
     the paperclip) at the front of the tools lane, shifting _modes from
     child 2 to child 4 — the positional anchor then hit the attachment
     button and forced its fixed 28px icon box into flex:0 1 auto /
     min-width:34px / max-width:max-content (broken attachment icon,
     2026-09-12). uV2eYG_modes is unique across every host package
     shipped so far; keep substring + guard audits on host upgrades. */
